...
pub const DEFAULT_SLIP0010_ED25519_PATH_FORMAT: &str = "m/44'/506'/0'/0'/{index}'";

pub fn default_path(index: u32) -> String {
    format!(DEFAULT_SLIP0010_ED25519_PATH_FORMAT, index = index)
}
...
pub fn derive_private_key(seed: &[u8], path: &[u32]) -> Result<[u8; 32]> {
    ...
    derive_ed25519_private_key(seed, path)
        .map_err(|err| anyhow!("slip0010 derivation failed: {err}"))
}
